"""System views built by the ROUTER (not user navigation): the boot spinner
...
and hands it to `SystemHubView`, which gives it the identical centered-card frame
...
class SystemHubView(BaseView):
    """A one-off view wrapping a pre-built content control (+ optional recovery
    actions) in the shared `BaseView` card frame. Used by the router for the boot
...
def boot_view(page: ft.Page, content: ft.Control) -> ft.View:
    """The calm full-screen boot spinner, in the shared card frame."""
